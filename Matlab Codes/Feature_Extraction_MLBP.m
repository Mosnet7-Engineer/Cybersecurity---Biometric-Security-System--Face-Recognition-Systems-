...
path_features = [pathtomainfolder 'Features/MLBP/'];
...
for i = 1:200 %% total users are 200
    for j = 1:3 %% total samples per user is 3
        ...
        figure,
        imshow(Current_Face)
        cd([pathtomainfolder 'Matlab Codes/MLBP/']) %% Path to LBP feature extraction code folder path
        Features_of_Current_Face = Compute_MLBP_Features(Current_Face)';
        %% Saving Features
        if j==1 
